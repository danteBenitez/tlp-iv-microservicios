...
import { IPurchaseRepository, purchaseRepository as purchaseRepository_ } from "../repositories/purchase.repository";
import { ProductService, productService as productService_ } from "./product.service";
import { SupplierService, supplierService as supplierService_ } from "./supplier.service";
...
export class PurchaseService {
    ...
    async findAll() {
        ...
        if (purchases?.length === 0) {
            throw new PurchaseNotFoundError("No hay compras registradas");
        }

        return purchases;
    }

    async buy(items: PurchaseDetailItem[], supplierId:string): Promise<{ purchase: IPurchase, total: number }> {

        const details = await Promise.all(items.map(async purchaseItem => {
            const product = await this.productService.findById(purchaseItem.productId);
            if (!product) {
    ...
    private async create(purchaseData: Omit<IPurchase, "purchaseId">, detailsData: Omit<IPurchaseDetail, "purchaseDetailId" | "purchaseId">[]): Promise<IPurchase> {
        const purchase = await this.purchaseRepository.create(purchaseData);

        await this.purchaseRepository.createDetails(purchase.purchaseId, detailsData);
